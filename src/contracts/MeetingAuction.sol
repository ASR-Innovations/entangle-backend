...
contract MeetingAuction is ReentrancyGuard, Ownable, Pausable, ERC721 {
    ...
    // Creator tokens (ERC20, held by contract), keyed by provided token name
    mapping(string => uint256) public creatorTokenBalances; // aggregate balance held by contract per token name
    // Creator address to their token name (immutable per creator once set)
    mapping(address => string) public creatorTokenName;
    // Creator address to deployed ERC20 token address
    mapping(address => address) public creatorTokenAddress;
    // Token name to deployed ERC20 token address
    ...
    event CreatorTokensMinted(string tokenName, address indexed minter, uint256 amount, address tokenAddress);
    ...
    function endAuction(uint256 _auctionId) external nonReentrant {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            // Calculate fees (10%) retained in contract; 90% to host
            uint256 platformAmount = (auction.highestBid * platformFee) / 10000; // 10%
            uint256 hostAmount = auction.highestBid - platformAmount;
 
            // Transfer host share; retain platformAmount in contract balance
            ...
            // If pool not created yet for this creator token, initialize pool using retained fees and all creator tokens
            string memory tName = creatorTokenName[auction.host];
            address tAddr = tokenNameToAddress[tName];
            if (poolManager != address(0) && tAddr != address(0)) {
                CreatorPoolManager pm = CreatorPoolManager(poolManager);
                // Check if pool exists
                if (!pm.poolExists(tAddr)) {
                    uint256 tokenAmt = creatorTokenBalances[tName];
                    if (tokenAmt > 0 && address(this).balance >= platformAmount) {
                        // Approve and transfer tokens to pool manager
                        (bool okApprove, ) = tAddr.call(abi.encodeWithSignature("approve(address,uint256)", poolManager, tokenAmt));
                        require(okApprove, "Approve failed");
                        (bool okTransfer, ) = tAddr.call(abi.encodeWithSignature("transfer(address,uint256)", poolManager, tokenAmt));
                        require(okTransfer, "Token transfer failed");
                        creatorTokenBalances[tName] = 0;
                        // Initialize pool with tokens and platformAmount native
                        pm.initPool{ value: platformAmount }(tAddr, tokenAmt);
                    }
                }
            }
        }
    ...
    function setCreatorTokenFactory(address _factory) external onlyOwner {
        require(_factory != address(0), "Invalid factory");
        creatorTokenFactory = _factory;
    }

    function setPoolManager(address _pm) external onlyOwner {
        require(_pm != address(0), "Invalid manager");
        poolManager = _pm;
    }
 
    // Simple mint creator ERC20 tokens (anyone can call). Tokens are minted to this contract.
    function mintCreatorTokens(string memory _tokenName, uint256 _amount) external nonReentrant whenNotPaused {
        require(bytes(_tokenName).length > 0, "Name required");
        require(_amount > 0, "Invalid amount");

        string memory existing = creatorTokenName[msg.sender];
        if (bytes(existing).length == 0) {
            creatorTokenName[msg.sender] = _tokenName;
        } else {
            require(keccak256(bytes(existing)) == keccak256(bytes(_tokenName)), "Name mismatch for creator");
        }

        address tokenAddr = creatorTokenAddress[msg.sender];
        if (tokenAddr == address(0)) {
            require(creatorTokenFactory != address(0), "Factory not set");
            string memory symbol = _deriveSymbol(_tokenName);
            tokenAddr = CreatorTokenFactory(creatorTokenFactory).createToken(_tokenName, symbol, address(this));
            creatorTokenAddress[msg.sender] = tokenAddr;
            tokenNameToAddress[_tokenName] = tokenAddr;
        }

        (bool success, ) = tokenAddr.call(abi.encodeWithSignature("mint(address,uint256)", address(this), _amount));
        require(success, "ERC20 mint failed");
        creatorTokenBalances[_tokenName] += _amount;

        emit CreatorTokensMinted(_tokenName, msg.sender, _amount, tokenAddr);
    }

    function _deriveSymbol(string memory _name) internal pure returns (string memory) {
        bytes memory n = bytes(_name);
        bytes memory out = new bytes(n.length > 8 ? 8 : n.length);
        uint256 j = 0;
        for (uint256 i = 0; i < n.length && j < 8; i++) {
            bytes1 c = n[i];
            if (c >= 0x61 && c <= 0x7A) {
                out[j++] = bytes1(uint8(c) - 32);
            } else if (
                (c >= 0x41 && c <= 0x5A) ||
                (c >= 0x30 && c <= 0x39)
            ) {
