...
/**
 * Sync all auctions from blockchain to database
 * This script will:
 * 1. Fetch all auctions from the database
...
async function syncAllAuctions() {
  console.log('🔄 STARTING FULL AUCTION SYNC FROM BLOCKCHAIN TO DATABASE');
  ...
  try {
    ...
    console.log(`📝 Contract: ${contractService.contractAddress}`);
    console.log('');
    ...
    const result = await pool.query('SELECT id FROM auctions ORDER BY id ASC');
    const auctionIds = result.rows.map(row => row.id);
    
    console.log(`📊 Found ${auctionIds.length} auctions in database`);
    ...
    if (successCount > 0) {
      ...
      console.log('1. ✅ Database is now in sync with blockchain');
...
console.log('');
console.log('╔════════════════════════════════════════════════════════════════════╗');
console.log('║         BLOCKCHAIN TO DATABASE SYNC SCRIPT                         ║');
